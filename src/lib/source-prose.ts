...
const BLOCK_TAGS = new Set([
  ...
  "h4", "h5", "h6", "header", "hr", "li", "main", "ol", "p", "pre", "section",
  "table", "tr", "ul",
]);
...
/**
 * Converts untrusted source-rich prose to display-safe plain text. This helper
 * deliberately has no DOM or React dependency so import, persistence, and
 * legacy presentation paths all apply the same policy.
 */
export function normalizeSourceProse(value: string | null | undefined): string | null {
...
export function normalizeCharacterProse<T extends {
  ...
  return {
    ...character,
    description: normalizeSourceProse(character.description),
    personality: normalizeSourceProse(character.personality),
    scenario: normalizeSourceProse(character.scenario),
    exampleDialogs: normalizeSourceProse(character.exampleDialogs),
  };
}
...
function stripMarkup(source: string): string {
  ...
  while (cursor < source.length) {
    ...
    // Image alternate text is upstream markup too; do not promote it into
    // canonical prose. Link and inline-formatting text remains in output.
    if (tag.name === "img") continue;
    if (tag.name === "br") output += "\n";
    else if (BLOCK_TAGS.has(tag.name)) output += "\n\n";
  }
...
function parseTag(source: string, start: number): HtmlTagToken | null {
  ...
  while (cursor < source.length) {
    ...
    if (quote) {
      ...
      return { closing, end: cursor + 1, name, selfClosing: source[contentEnd] === "/" };
    }
    cursor += 1;
  }
  return null;
...
function parseSpecialTag(source: string, start: number): HtmlTagToken | null {
  ...
  return end === -1 ? null : { closing: false, end: end + 1, name: "", selfClosing: true };
}

function looksLikeUnclosedRemovedContentTag(source: string, start: number): boolean {
  return /^<\/?(?:script|style|template|noscript)\b/i.test(source.slice(start));
}

const NAMED_ENTITIES: Readonly<Record<string, string>> = {
  amp: "&", apos: "'", bull: "•", cent: "¢", copy: "©", divide: "÷", euro: "€",
  gt: ">", hellip: "…", ldquo: "“", lsquo: "‘", lt: "<", mdash: "—", middot: "·",
  nbsp: "\u00a0", ndash: "–", pound: "£", quot: '"', rdquo: "”", reg: "®",
  rsquo: "’", times: "×", yen: "¥",
};
