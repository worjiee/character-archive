...
export function CharacterManagementPanel({ character }: { character: CharacterDetail }) {
  ...
  return (
    <section className="mt-10 rounded-xl border border-violet-500/20 bg-zinc-900/45 p-5 sm:p-6">
      <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-violet-400">Owner tools</p><h2 className="mt-2 text-lg font-semibold text-zinc-100">Character management</h2><p className="mt-1 text-sm text-zinc-500">Local presentation overrides survive future source imports.</p></div>
      ...
      <details open className="mt-5 rounded-lg border border-zinc-800 bg-zinc-950/35"><summary className="cursor-pointer px-4 py-3 text-sm font-medium text-zinc-200">Local display overrides</summary><form onSubmit={saveOverrides} className="grid gap-4 border-t border-zinc-800 p-4">
      ...
      <details className="mt-3 rounded-lg border border-zinc-800 bg-zinc-950/35"><summary className="cursor-pointer px-4 py-3 text-sm font-medium text-zinc-200">Greeting presentation ({character.greetings.length})</summary><div className="space-y-3 border-t border-zinc-800 p-4">{character.greetings.map((greeting, index) => <div key={greeting.id} className={`rounded-lg border border-zinc-800 p-3 ${greeting.hidden ? "opacity-50" : ""}`}><p className="line-clamp-2 text-xs leading-5 text-zinc-400">{greeting.content}</p><div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" aria-label={`Move greeting ${index + 1} up`} disabled={busy !== null || index === 0} onClick={() => void moveGreeting(index, -1)} className={smallButton}>↑</button><button type="button" aria-label={`Move greeting ${index + 1} down`} disabled={busy !== null || index === character.greetings.length - 1} onClick={() => void moveGreeting(index, 1)} className={smallButton}>↓</button><button type="button" disabled={busy !== null} onClick={() => void request(`visibility-${greeting.id}`, `/api/characters/${character.id}/greetings`, jsonRequest("PATCH", { action: "visibility", greetingId: greeting.id, hidden: !greeting.hidden }), greeting.hidden ? "Greeting shown." : "Greeting hidden.")} className={secondaryButton}>{greeting.hidden ? "Show" : "Hide"}</button><span className="ml-auto text-[11px] text-zinc-600">Local order {index + 1}</span></div></div>)}</div></details>
