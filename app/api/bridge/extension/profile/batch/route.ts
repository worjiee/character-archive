import { BridgeError, bridgeErrorResponse, companionPreflight, readBoundedBridgeJson, receiveProfileCharacterBatch, requireCompanionOrigin, withCompanionCors } from "@/src/lib/bridge";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await receiveProfileCharacterBatch(token, await readBoundedBridgeJson(request, 11 * 1024 * 1024), new URL(request.url).origin, { capabilityOrigin: companionOrigin });
